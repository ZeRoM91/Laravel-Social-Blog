...
@section('content')
    <ol class="breadcrumb">
        ...
        <li>
            <a href="{{route('homeCategory', ['category_id' => $article['category_id']])}}">{{$article->category['name']}}</a>
        </li>
    ...
    <div class="well">
        ...
        <br> @if(Auth::user()->id == $article->user_id)
            ...
            @foreach($articles as $item)
                <a href="{{route('article.show', $item)}}"><p><b>{{$item->id}}. {{$item->title}}</b></p></a>
            @endforeach
        @endif
        <div class="panel panel-default">
            <div class="panel-body">
                @if($article['rating']  > 0)
                    <span><b>Рейтинг статьи: <span
                                    class="label label-success">+{{$article['rating']}}</span> </b></span>
                @endif
                @if($article['rating'] < 0)
                ...
                @if(Auth::user()->id != $article['user_id'])
                    ...
                    @if($vote['vote'] === 1)
                        <div class="btn-group" role="group" aria-label="...">
                            <a href="{{route('resetRating',['id' => $article->id])}}" class="btn btn-primary"
                               title="Отменить голос">Отменить</a>
        ...
        @if(Auth::user()->id == $article['user_id'])
            ...
            <button class="btn btn-danger" id="article__delete" data-toggle="modal" data-target="#myModal">Удалить
            </button>
        @endif</div>
    ...
    <form method="post" id="comment-form">
        ...
        <textarea id="comment" class="form-control" name="comment" cols="90" rows="4"
    ...
    <div id="comment-box">
        ...
        @foreach($comments as $comment)
            ...
            <img class="img-circle"
                 src="{{isset($comment->user->avatar) ? asset('storage/avatars/' . $comment->user->avatar) : '/img/avatar.jpg'}}">
    ...
    <div class="modal fade bs-example-modal-sm" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel"
        ...
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content" style="padding: 20px;">
                ...
                <hr>
                <a href="{{route('article.destroy',$article)}}" class="btn btn-default">Да</a>
