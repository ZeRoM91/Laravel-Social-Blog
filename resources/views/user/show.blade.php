                ...
                @if(isset($isFriend))
                    @if($isFriend -> pivot -> status === 1)
                        <a href="{{route('messages__user', ['id' => $isFriend -> pivot -> to_user_id])}}">
                            <button type="button" class="btn btn-success"><span class="glyphicon glyphicon-send"></span>
                                Сообщение
                            </button>
                        </a>
                        <hr>
                        ...
                        <div class="btn-group">
                            ...
                            <ul class="dropdown-menu">
                                <li>
                                    <a href="{{route('user__friend-decline', ['id' => $isFriend -> pivot -> to_user_id])}}">
                                        <button class="btn btn-danger"><span class="glyphicon glyphicon-remove"></span>Удалить
                                            из друзей
                                        </button>
                                    </a></li>
                    ...
                    @if($isFriend -> pivot -> status === 0)
                        <button class="btn btn-info" disabled><span class="glyphicon glyphicon-ok"></span>Запрос
                            отправлен
                        </button>
                    @endif
                @else
